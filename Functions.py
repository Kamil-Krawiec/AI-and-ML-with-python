...
def dijkstra_time(graph, start, goal, time):
    ...
    decode_path(came_from, start=start, goal=goal)


# ----------------------------------------- ASTAR

def time_heuristic(goal, next_stop):
    return (abs(goal.start_stop_lat - next_stop.start_stop_lat) + abs(goal.start_stop_lon - next_stop.start_stop_lon))*100
